...
const equationCleanup = (equation: string): string => {
  let formattedEquation = equation.trim().replaceAll(" ", "").toLowerCase();
...
const truthArray = (equation: string) => {
  ...
  let result = calculateTable(equation, vars, boolsArrs, stack, charMap);
  let len: number = 2 ** vars.length;
  let truthArr: JSX.Element[][] = [];
  for (let i = 0; i < len; i++) {
    truthArr.push([]);
    charMap.forEach((elem) => truthArr[i].push(<td>{elem[i].toString()}</td>));
    try {
      //TODO:: REMOVE THIS
